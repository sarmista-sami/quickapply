...
The system SHALL maintain a declarative map from `ApplicantData` paths to Workday
`data-automation-id` selectors for text fields, in the Workday site adapter at the edge.
The map SHALL cover the account/sign-in email (`formField-email`) and the My Information
first name, last name, phone, and address line. Because the adapter fills only fields
present on the page, one map serves both pages without branching.
